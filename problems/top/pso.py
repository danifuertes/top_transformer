...
class PSO:
    def __init__(self, uav_num, target_num, targets, vehicles_speed, time_lim, return2depot=False):
            ...
            for j in range(k):
                ...
                if j == 0:
                    ...
                    t = t + self.Distance[m1][m2] / \
                        self.vehicles_speed[i] + self.Stay_time[m1]
                t_flyback = self.Distance[m1][0] if self.return2depot else 0
                if t + t_flyback <= self.time_all:
            ...
            for j in range(k):
                ...
                if j == 0:
                    ...
                    t = t + self.Distance[m2][m1] / \
                        self.vehicles_speed[i] + self.Stay_time[m1]
                t_flyback = self.Distance[m1][0] / self.vehicles_speed[i] if self.return2depot else 0
                if t + t_flyback <= self.time_all:
                    UAV_Out[i].append(m1)
                    self.time_out[i] = t + t_flyback
        return UAV_Out
