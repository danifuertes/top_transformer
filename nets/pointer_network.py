...
class PointerNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        # Encoder LSTM
        self.lstm_enc = Encoder(embedding_dim, hidden_dim)

        # Decoder LSTM
        self.lstm_dec = nn.LSTMCell(embedding_dim, hidden_dim)
    ...
    def _inner(self, inputs, eval_tours=None):
        ...
        # Initialize hidden state of encoder and decoder LSTMs, and get context
        h0 = c0 = Variable(
            torch.zeros(1, batch_size, self.num_agents, self.hidden_dim, out=node_embedding.data.new()),
            requires_grad=False
        )  # Variable is deprecated, use Tensor
        context, enc_h0, enc_c0 = tuple(), tuple(), tuple()
        for k in range(self.num_agents):
            context_agent, (enc_h0_agent, enc_c0_agent) = self.lstm_enc(
                node_embedding,
                (h0[:, :, k].contiguous(), c0[:, :, k].contiguous())
            )
            context = context + (context_agent[:, :, None], )
            enc_h0 = enc_h0 + (enc_h0_agent[:, :, None], )
            enc_c0 = enc_c0 + (enc_c0_agent[:, :, None], )
        context = torch.cat(context, dim=2)
        enc_h0 = torch.cat(enc_h0, dim=2)
        enc_c0 = torch.cat(enc_c0, dim=2)
        hidden_enc = (enc_h0, enc_c0)
        hidden_dec = (enc_h0[-1], enc_c0[-1])
        ...
        while not state.all_finished():
            ...
            # For each agent
            h_aux, log_p, probs = tuple(), tuple(), tuple()
            for k in range(self.num_agents):

                # LSTM (decoder)
                h, c = self.lstm_dec(x[:, k], (hidden_dec[0][:, k], hidden_dec[1][:, k]))
                if k == 0:
                    h_aux = (h[:, None], c[:, None])
                else:
                ...
                for i in range(self.n_glimpses):
                    ref, logits = self.glimpse(h, context[:, :, k])
                ...
                _, logits = self.pointer(h, context[:, :, k])  # query = h, ref = context
            ...
            # Calculate the context (with the encoder LSTM) again
            context, enc_h0, enc_c0 = tuple(), tuple(), tuple()
            for k in range(self.num_agents):
                context_agent, (enc_h0_agent, enc_c0_agent) = self.lstm_enc(
                    node_embedding,
                    (hidden_enc[0][:, :, k].contiguous(), hidden_enc[1][:, :, k].contiguous())
                )
                context = context + (context_agent[:, :, None],)
                enc_h0 = enc_h0 + (enc_h0_agent[:, :, None],)
                enc_c0 = enc_c0 + (enc_c0_agent[:, :, None],)
