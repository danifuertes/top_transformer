...
def eval_dataset(dataset_path, width, softmax_temp, opts):
    ...
    model, args = load_model(opts.model, num_agents=opts.num_agents)
    ...
    model_name = os.path.join(args['model'], "_".join(os.path.normpath(os.path.splitext(opts.model)[0]).split(os.sep)[-2:]))
    if opts.o is None:
...
def _eval_dataset(model, dataset, width, softmax_temp, opts, device):
    ...
    end_ids = dataset[0]['loc'].shape[0] + 1
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        for seq, cost in zip(sequences, costs):
            ...
            elif model.problem.NAME in ("op", "pctsp", "top"):
                new_seq = []
                for k in range(opts.num_agents):
                    new_seq.append(np.delete(seq[:, k], np.argwhere(seq[:, k] == end_ids)))  # We usually exclude depot
                seq = new_seq
            else:
...
if __name__ == "__main__":
    ...
    # Model
    parser.add_argument('--model', type=str, help='Path to the model to be evaluated')
    ...
    parser.add_argument('--width', type=int, nargs='+',  help='Sizes of beam to use for beam search '
                        '(or number of samples for sampling), 0 to disable (default), -1 for infinite')
